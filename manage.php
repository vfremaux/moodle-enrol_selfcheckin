...
$instance = $DB->get_record('enrol', array('id' => $enrolid, 'enrol' => 'selfcheckin'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id' => $instance->courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id, MUST_EXIST);

require_login($course);
$canenrol = has_capability('enrol/selfcheckin:enrol', $context);
$canunenrol = has_capability('enrol/selfcheckin:unenrol', $context);
...
if (!$canenrol and !$canunenrol) {
    ...
    require_capability('enrol/selfcheckin:unenrol', $context);
}
...
if (!$enrol_selfcheckin = enrol_get_plugin('selfcheckin')) {
    throw new coding_exception('Can not instantiate enrol_manual');
}

$instancename = $enrol_selfcheckin->get_instance_name($instance);

$PAGE->set_url('/enrol/selfcheckin/manage.php', array('enrolid' => $instance->id));
$PAGE->set_pagelayout('admin');
$PAGE->set_title($enrol_selfcheckin->get_instance_name($instance));
...
$potentialuserselector = new enrol_selfcheckin_potential_participant('addselect', $options);
$potentialuserselector->viewfullnames = $viewfullnames;
$currentuserselector = new enrol_selfcheckin_current_participant('removeselect', $options);
...
if (empty($extendbase)) {
    if (!$extendbase = get_config('enrol_selfcheckin', 'enrolstart')) {
...
$basemenu[4] = get_string('now', 'enrol_selfcheckin') . ' (' . userdate($now, get_string('strftimedatetimeshort')) . ')';
...
if ($canenrol && optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        foreach($userstoassign as $adduser) {
            ...
            $enrol_selfcheckin->enrol_user($instance, $adduser->id, $roleid, $timestart, $timeend);
        }
    ...
    <tr>
      ...
      <td id="buttonscell">
          <div id="addcontrols">
              ...
              <p><label for="menuroleid"><?php print_string('assignrole', 'enrol_selfcheckin') ?></label><br />
