...
class enrol_selfcheckin_plugin extends enrol_plugin {
    ...
    public function allow_enrol(stdClass $instance) {
        // Users with enrol cap may enrol other users manually with self check in enrol.
        return true;
    }

    public function allow_unenrol(stdClass $instance) {
        // Users with unenrol cap may unenrol other users self checked in manually.
        return true;
    }
    ...
    public function get_manual_enrol_link($instance) {
        ...
        if (!has_capability('enrol/selfcheckin:enrol', $context)) {
            // Note: manage capability not used here because it is used for editing
            // of existing enrolments which is not possible here.
            return NULL;
        }

        return new moodle_url('/enrol/selfcheckin/manage.php', array('enrolid' => $instance->id, 'id' => $instance->courseid));
    }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/selfcheckin:config', $context)) {
            return false;
        }

        if ($DB->record_exists('enrol', array('courseid' => $courseid, 'enrol' => 'selfcheckin'))) {
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        $icons = array();
        if (has_capability('enrol/selfcheckin:enrol', $context) or has_capability('enrol/selfcheckin:unenrol', $context)) {
            $managelink = new moodle_url("/enrol/selfcheckin/manage.php", array('enrolid' => $instance->id));
            $icons[] = $OUTPUT->action_icon($managelink, new pix_icon('t/enrolusers', get_string('enrolusers', 'enrol_selfcheckin'), 'core', array('class' => 'iconsmall')));
        }
    ...
    */
    ...
    public function add_instance($course, array $fields = NULL) {
        ...
        if ($DB->record_exists('enrol', array('courseid' => $course->id, 'enrol' => 'selfcheckin'))) {
    ...
    public function update_instance($instance, $data) {
        ...
        if ($instances = $DB->get_records('enrol', array('courseid' => $instance->courseid, 'enrol' => 'selfcheckin'), 'id ASC')) {
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        if (!enrol_is_enabled('selfcheckin')) {
            $trace->finished();
            return 2;
        }
        ...
        if ($action == ENROL_EXT_REMOVED_UNENROL) {
            ...
            $sql = "SELECT ue.*, e.courseid, c.id AS contextid
                      ...
                      JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'selfcheckin')
        ...
        $trace->output('...self check in enrolment updates finished.');
        $trace->finished();
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/selfcheckin:manage')) {
    ...
    public function restore_instance(restore_enrolments_structure_step $step, stdClass $data, $course, $oldid) {
        ...
        if ($instances = $DB->get_records('enrol', array('courseid' => $data->courseid, 'enrol' => 'selfcheckin'), 'id')) {
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/selfcheckin:config', $context);
    }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_selfcheckin'), $options);
        $mform->addHelpButton('status', 'status', 'enrol_selfcheckin');
        ...
        $mform->addElement('duration', 'enrolperiod', get_string('defaultperiod', 'enrol_selfcheckin'), $options);
        $mform->setDefault('enrolperiod', $this->get_config('enrolperiod'));
        $mform->addHelpButton('enrolperiod', 'defaultperiod', 'enrol_selfcheckin');
 ...
 * @return string
 */
function enrol_selfcheckin_output_fragment_enrol_users_form($args) {
    $args = (object) $args;
    $context = $args->context;
    $o = '';

    require_capability('enrol/selfcheckin:enrol', $context);
    $mform = new enrol_selfcheckin_enrol_users_form(null, $args);
