 ...
 *
 * @module     enrol_self_check_in/quickenrolment
...
const Selectors = {
    ...
    triggerButtons: ".enrolusersbutton.enrol_self_check_in_plugin [type='submit']",
...
const getBodyForContext = contextId => {
    return Fragment.loadFragment('enrol_self_check_in', 'enrol_users_form', contextId, {});
};
...
const showModal = (dynamicTable, contextId) => {
    const pendingPromise = new Pending('enrol_self_check_in/quickenrolment:showModal');

    return ModalFactory.create({
        type: ModalFactory.types.SAVE_CANCEL,
        large: true,
        title: Str.get_string('enrolusers', 'enrol_self_check_in'),
        body: getBodyForContext(contextId),
        buttons: {
            save: Str.get_string('enrolusers', 'enrol_self_check_in'),
        }
    })
    .then(modal => {
    ...
    .then(([modal, body]) => {
        if (body.get(0).querySelector(Selectors.cohortSelector)) {
            return modal.setSaveButtonText(Str.get_string('enroluserscohorts', 'enrol_self_check_in')).then(() => modal);
        }
...
const submitFormAjax = (dynamicTable, modal) => {
    ...
    jQuery.ajax(
        `${Config.wwwroot}/enrol/self_check_in/ajax.php?${form.serialize()}`,
        {
            type: 'GET',
            processData: false,
...
/**
 * Set up quick enrolment for the self check in enrolment plugin.
 *
...
export const init = ({contextid}) => {
    ...
    Prefetch.prefetchStrings('enrol_self_check_in', [
        'enrolusers',
        'enroluserscohorts',
    ]);
