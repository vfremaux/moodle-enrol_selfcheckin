...
/**
 * Self Check In external functions.
 *
 * @package    enrol_selfcheckin
 * @category   external
 * @copyright  2011 Jerome Mouneyrac
 ...
 * @since Moodle 2.2
 */
class enrol_selfcheckin_external extends external_api {
    ...
    public static function enrol_users($enrolments) {
        ...
        // Retrieve the selfcheckin enrolment plugin.
        $enrol = enrol_get_plugin('selfcheckin');
        if (empty($enrol)) {
            throw new moodle_exception('selfcheckinpluginnotinstalled', 'enrol_selfcheckin');
        }
        ...
        foreach ($params['enrolments'] as $enrolment) {
            ...
            // Check that the user has the permission to selfcheckin enrol.
            ...
            if (!array_key_exists($enrolment['roleid'], $roles)) {
                ...
                throw new moodle_exception('wsusercannotassign', 'enrol_selfcheckin', '', $errorparams);
            }

            // Check self check in enrolment plugin instance is enabled/exist.
            $instance = null;
            $enrolinstances = enrol_get_instances($enrolment['courseid'], true);
            foreach ($enrolinstances as $courseenrolinstance) {
              if ($courseenrolinstance->enrol == "selfcheckin") {
            ...
            if (empty($instance)) {
              $errorparams = new stdClass();
              $errorparams->courseid = $enrolment['courseid'];
              throw new moodle_exception('wsnoinstance', 'enrol_self_chek_in', $errorparams);
            }
            ...
            if (!$enrol->allow_enrol($instance)) {
                ...
                $errorparams->courseid = $enrolment['courseid'];
                $errorparams->userid = $enrolment['userid'];
                throw new moodle_exception('wscannotenrol', 'enrol_selfcheckin', '', $errorparams);
            }
    ...
    public static function unenrol_users($enrolments) {
        ...
        foreach ($params['enrolments'] as $enrolment) {
            ...
            $instance = $DB->get_record('enrol', array('courseid' => $enrolment['courseid'], 'enrol' => 'selfcheckin'));
            if (!$instance) {
                throw new moodle_exception('wsnoinstance', 'enrol_selfcheckin', $enrolment);
            }
            ...
            if (!$enrol->allow_unenrol($instance)) {
                throw new moodle_exception('wscannotunenrol', 'enrol_selfcheckin', '', $enrolment);
            }
