...
/**
 * This file processes AJAX enrolment actions and returns JSON for the self check in enrolments plugin
 *
...
require_once($CFG->dirroot.'/enrol/selfcheckin/classes/enrol_users_form.php');
...
switch ($action) {
    case 'enrol':
        ...
        if (empty($startdate)) {
            if (!$startdate = get_config('enrol_selfcheckin', 'enrolstart')) {
        ...
        $mform = new enrol_selfcheckin_enrol_users_form(null, (object)["context" => $context]);
        $userenroldata = [
                'startdate' => $timestart,
                'timeend' => $timeend,
        ];
