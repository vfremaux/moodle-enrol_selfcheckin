...
/**
 * Auxiliary self cehck in user enrolment lib, the main purpose is to lower memory requirements...
 *
 ...
 * Enrol candidates.
 */
class enrol_selfcheckin_potential_participant extends user_selector_base {
    protected $enrolid;
    ...
    protected function get_options() {
        ...
        $options['file']    = 'enrol/selfcheckin/locallib.php';
        return $options;
    }
}
 ...
 * Enrolled users.
 */
class enrol_selfcheckin_current_participant extends user_selector_base {
    protected $courseid;
    protected $enrolid;
...
/**
 * A bulk operation for the self check in enrolment plugin to edit selected users.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_selfcheckin_editselectedusers_operation extends enrol_bulk_enrolment_operation {
    ...
    public function get_title() {
        return get_string('editselectedusers', 'enrol_selfcheckin');
    }
    ...
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        ...
        if (!has_capability("enrol/selfcheckin:manage", $manager->get_context())) {
            return false;
        }
     ...
     * @return enrol_selfcheckin_editselectedusers_form
     */
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        global $CFG;
        require_once($CFG->dirroot.'/enrol/selfcheckin/bulkchangeforms.php');
        return new enrol_selfcheckin_editselectedusers_form($defaultaction, $defaultcustomdata);
    }
}


/**
 * A bulk operation for the self check in enrolment plugin to delete selected users enrolments.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_selfcheckin_deleteselectedusers_operation extends enrol_bulk_enrolment_operation {
     ...
     */
    public function get_title() {
        return get_string('deleteselectedusers', 'enrol_selfcheckin');
    }
    ...
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        ...
        $defaultcustomdata['message'] = get_string('confirmbulkdeleteenrolment', 'enrol_selfcheckin');
        $defaultcustomdata['button'] = get_string('unenrolusers', 'enrol_selfcheckin');
        return new enrol_selfcheckin_deleteselectedusers_form($defaultaction, $defaultcustomdata);
    }
     ...
     */
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        if (!has_capability("enrol/selfcheckin:unenrol", $manager->get_context())) {
            return false;
        }
        $counter = 0;
        foreach ($users as $user) {
...
/**
 * Migrates all enrolments of the given plugin to enrol_selfcheckin plugin,
 ...
 */
function enrol_selfcheckin_migrate_plugin_enrolments($enrol) {
    global $DB;

    if ($enrol === 'selfcheckin') {
        // We can not migrate to self.
        return;
    }

    $selfcheckinplugin = enrol_get_plugin('selfcheckin');
    ...
    $sql = "
        ...
        LEFT JOIN
            {enrol} me ON (me.courseid = e.courseid AND me.enrol='selfcheckin')
        WHERE
            e.enrol = :enrol
        GROUP BY
            e.id, e.courseid, e.status
        ORDER BY
            e.id
    ";
    ...
    foreach($rs as $e) {
        $minstance = false;
        if (!$e->mid) {
            // Self check in instance does not exist yet, add a new one.
            $course = $DB->get_record('course', array('id' => $e->courseid), '*', MUST_EXIST);
            if ($minstance = $DB->get_record('enrol', array('courseid' => $course->id, 'enrol' => 'selfcheckin'))) {
                // Already created by previous iteration.
                $e->mid = $minstance->id;
            } else if ($e->mid = $selfcheckinplugin->add_default_instance($course)) {
        ...
        if (!$minstance) {
            ...
            debugging('Failed to find self check in enrolment instance', DEBUG_DEVELOPER);
            continue;
        }
        ...
        $sql = "
            ...
            ON
                (mra.contextid = ra.contextid AND
                mra.userid = ra.userid AND
                mra.roleid = ra.roleid AND
                mra.component = :empty AND
                mra.itemid = 0)
            WHERE
                ra.component = :component AND
                ra.itemid = :id
        ";
        ...
        // Migrate to self check in enrol instance.
        $params = array('id' => $e->id, 'mid' => $e->mid);
 ...
 */
function enrol_selfcheckin_get_potential_cohorts($context, $enrolid, $search = '', $page = 0, $perpage = 25, $addedenrollment = 0) {
    global $CFG;
