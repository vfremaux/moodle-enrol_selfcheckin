 ...
 * Enrol users form.
 *
 * Simple form to search for users and add them using a self check in enrolment to this course.
 *
...
class enrol_selfcheckin_enrol_users_form extends moodleform {
    ...
    public function definition() {
        ...
        foreach ($manager->get_enrolment_instances() as $tempinstance) {
            if ($tempinstance->enrol == 'selfcheckin') {
                if ($instance === null) {
        ...
        $options = array(
            'ajax' => 'enrol_selfcheckin/form-potential-user-selector',
            'multiple' => true,
            'courseid' => $course->id,
            'enrolid' => $instance->id,
        ...
        $mform->addElement('autocomplete', 'userlist', get_string('selectusers', 'enrol_selfcheckin'), array(), $options);
        ...
        if (has_capability('moodle/cohort:manage', $context) || has_capability('moodle/cohort:view', $context)) {
            ...
            if (!empty($availablecohorts)) {
                ...
                $mform->addElement('cohort', 'cohortlist', get_string('selectcohorts', 'enrol_selfcheckin'), $options);
            }
        }

        $roles = get_assignable_roles($context);
        $mform->addElement('select', 'roletoassign', get_string('assignrole', 'enrol_selfcheckin'), $roles);
